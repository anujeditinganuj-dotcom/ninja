...
from config import API_HASH, API_ID, BOT_TOKEN, LOG_CHANNEL_ID, MAINTENANCE_REMINDER_HOURS, OWNER_ID, PORT
...
from services.settings_service import get as get_setting, load_settings
from utils.blockquote import apply_blockquote_patch
from utils.keep_alive import keep_alive_loop
from utils.log_channel import attach as attach_log_channel, log_event
...
async def load_and_start_bots():
    # Settings → Bot Management → Auto Start Bots gates this whole step —
    # was being saved to the DB correctly but never actually read, so
    # turning it OFF had no effect and bots kept reconnecting on every boot.
    if not get_setting("auto_start_bots"):
        logger.info("Auto Start Bots is OFF (Settings) — skipping reconnect on boot.")
        return
    ...
    async for bot_doc in cursor:
        ...
        elif get_setting("notify_errors"):
            # Settings → Notifications → Error Alerts gates this DM — same
            # bug as above, this used to fire unconditionally regardless of
            # the toggle.
...
async def main():
    ...
    attach_log_channel(master, LOG_CHANNEL_ID)
    await log_event("🚀 <b>BotControl Manager started</b>")
