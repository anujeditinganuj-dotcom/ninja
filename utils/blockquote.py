...
# Telegram inline-formatting tags a handler may have deliberately written
# into a message (e.g. the "Powered by <a href="...">Anuj Kumar</a>" link in
# handlers/admin.py). These must survive BQ()'s escaping untouched, or
# Telegram just prints the literal tag text instead of parsing it. Anything
# NOT matching this whitelist (stray "<"/">"/"&" from dynamic content like a
# user's first_name) still gets escaped below, so it can't break out of the
# surrounding <blockquote>.
...
def BQ(text):
    """Escape then wrap text in a Telegram HTML blockquote, preserving any
    whitelisted formatting tags already in the text (see _SAFE_TAG_RE).
    None/empty text passes through unchanged so media-only sends with no
    caption still work."""
    if not text:
        return text
    text = str(text)
    ...
    for m in _SAFE_TAG_RE.finditer(text):
        chunks.append(_html.escape(text[pos:m.start()], quote=False))
        chunks.append(m.group())  # whitelisted tag — leave exactly as-is
        pos = m.end()
    chunks.append(_html.escape(text[pos:], quote=False))
    return f"<blockquote>{''.join(chunks)}</blockquote>"
