...
def register(app: Client) -> None:
    ...
    async def settings_about_cb(client: Client, cq: CallbackQuery):
        ...
        bot_link = f"https://t.me/{me.username}" if me.username else "https://t.me/"
        about_text = (
            f"💠 {SC('About This Bot')} 💠\n\n"
            f"╭────[ ✨ {SC('Anuj')} ]────⍟\n"
            f"├⍟ 🚀 {SC('Bot Name')}  : [{SC('BotControl Manager')}]({bot_link})\n"
            f"├⍟ 👨‍💻 {SC('Developer')}  : [{SC('Anuj Kumar')}](https://t.me/anujedits97)\n"
            f"├⍟ 🔗 {SC('Library')}  : [{SC('Pyrogram Async')}](https://docs.pyrogram.org/)\n"
            f"├⍟ ⚡️ {SC('Language')}  : [{SC('Python')} 3.11+](https://www.python.org/)\n"
            f"├⍟ ⚙️ {SC('Database')}  : [{SC('MongoDB')}](https://www.mongodb.com/)\n"
            f"├⍟ ⭐️ {SC('Hosting')}  :  {SC('Dedicated High-Speed VPS')}\n"
            "╰───────────────⍟"
        )
        await cq.message.edit_text(about_text, reply_markup=back_keyboard("settings_menu"))
        await cq.answer()
