...
def register(app: Client) -> None:
    ...
    async def broadcast_receive(client: Client, message: Message):
        ...
        media_type = raw_file_id = None
        if message.photo:
            media_type, raw_file_id = "photo", message.photo.file_id
        elif message.video:
            media_type, raw_file_id = "video", message.video.file_id
        elif message.document:
            media_type, raw_file_id = "document", message.document.file_id
        elif message.animation:
            media_type, raw_file_id = "animation", message.animation.file_id

        if media_type:
            # raw_file_id was captured on the MASTER bot's chat and is not
            # valid for the managed bot's own client to resend — re-cache it
            # through that bot first (see BotManager.recache_media).
            cached_id = await bot_manager.recache_media(bot_id, media_type, raw_file_id, client, message.from_user.id)
            if cached_id is None:
                return await message.reply_text(
                    "⚠️ This bot isn't connected right now, so the media couldn't be cached for it.\n"
                    "Reconnect it, then resend the broadcast content."
                )
            content = {"type": media_type, "file_id": cached_id, "caption": message.caption}
        else:
