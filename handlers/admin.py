...
async def send_dashboard(client: Client, chat_id: int, first_name: str = "User") -> None:
    """Sends the welcome photo with the dashboard text as its caption, in one
    message — used for /start and every "« Back" that returns to the main
    dashboard. Photo captions are capped at 1024 chars by Telegram, which
    WELCOME_TEXT + the live stats block used to exceed (MEDIA_CAPTION_TOO_LONG).
    WELCOME_TEXT has been trimmed to stay safely under that cap; if new lines
    are ever added here, keep an eye on the combined length."""
