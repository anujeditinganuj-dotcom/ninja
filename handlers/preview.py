...
def register(app: Client) -> None:
    ...
    async def preview_cb(client: Client, cq: CallbackQuery):
        ...
        # Stored media file_ids are cached specifically for THIS managed
        # bot's own client (see BotManager.recache_media) — the master
        # bot's client can't resend them, so use the managed bot's client
        # for media previews. Text-only previews still work fine either way.
        managed_client = bot_manager.clients.get(bot_id)
        try:
            if media_type and file_id and not managed_client:
                await cq.message.reply_text(
                    "⚠️ This bot isn't connected right now, so media can't be previewed "
                    f"— showing the text/caption only:\n\n{text}"
                )
            elif media_type == "photo" and file_id:
                await managed_client.send_photo(chat_id, file_id, caption=caption, reply_markup=markup)
            elif media_type == "video" and file_id:
                await managed_client.send_video(chat_id, file_id, caption=caption, reply_markup=markup)
            elif media_type == "document" and file_id:
                await managed_client.send_document(chat_id, file_id, caption=caption, reply_markup=markup)
            elif media_type == "animation" and file_id:
                await managed_client.send_animation(chat_id, file_id, caption=caption, reply_markup=markup)
            else:
