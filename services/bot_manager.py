...
async def _make_blurred_image(client, file_id: str):
    ...
    try:
        ...
        # Step 2: Add glitter/sparkle grain (white bright dots like CORN HUB style)
        try:
            arr = _np.array(img, dtype=_np.int16)

            # Base noise (film grain texture)
            noise = _np.random.randint(-12, 12, arr.shape, dtype=_np.int16)
            arr = _np.clip(arr + noise, 0, 255).astype(_np.uint8)

            # Sparkle dots — random bright white pixels scattered
            img_temp = Image.fromarray(arr)
            from PIL import ImageDraw
            draw = ImageDraw.Draw(img_temp)
            w, h = img_temp.size
            num_sparkles = int((w * h) * 0.003)  # ~0.3% of pixels = sparkles
            for _ in range(num_sparkles):
                x = int(_np.random.randint(0, w))
                y = int(_np.random.randint(0, h))
                size = int(_np.random.choice([1, 1, 1, 2]))  # mostly 1px, some 2px
                brightness = int(_np.random.randint(200, 255))
                draw.ellipse(
    ...
    async def _send_content(self, client: Client, chat_id: int, content: dict, buttons: list, bot_id: int = None) -> None:
        ...
        try:
            if media_type == "photo" and file_id:
                try:
                    if photo_spoiler:
                        # Single photo with Telegram spoiler (tap to reveal original)
                        # PIL blur overlay not possible in single-photo reveal flow
