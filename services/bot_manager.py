...
class BotManager:
    ...
    async def start_bot(self, bot_doc: dict) -> tuple[bool, str]:
        ...
        async with self._lock(bot_id):
            ...
            await log_event(f"🟢 <b>Bot connected</b>\n@{bot_doc.get('username', bot_id)}")
            return True, "started"
    ...
    async def _send_content(self, client: Client, chat_id: int, content: dict, buttons: list, bot_id: int = None) -> None:
        ...
        try:
            if media_type == "photo" and file_id:
                try:
                    await client.send_photo(chat_id, file_id, caption=caption, reply_markup=markup, parse_mode=enums.ParseMode.HTML, has_spoiler=photo_spoiler)
