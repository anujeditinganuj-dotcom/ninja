...
def join_channel_keyboard(channels) -> InlineKeyboardMarkup:
    ...
    Left un-styled/un-smallcapped on purpose: this is shown to the managed
    bot's *end users*, not the owner's control panel."""
    if isinstance(channels, str):
        channels = [("📢 Join Channel", channels)]
    rows = [[InlineKeyboardButton(f"📢 {label}", url=url)] for label, url in channels]
    rows.append([InlineKeyboardButton("✅ I've Joined", callback_data="fs_recheck")])
    return InlineKeyboardMarkup(rows)
