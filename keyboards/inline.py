...
def photo_type_keyboard(bot_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [
            make_button("🗑 Remove Normal Photo", callback_data=f"delphoto_normal:{bot_id}", style=BTN_DANGER),
            make_button("🗑 Remove Maint. Photo", callback_data=f"delphoto_maint:{bot_id}", style=BTN_DANGER),
        ],
...
def buttons_manage_keyboard(bot_id: int, buttons: list) -> InlineKeyboardMarkup:
    ...
    for r, row in enumerate(buttons or []):
        for c, b in enumerate(row):
            ...
            preview_style, _ = BUTTON_COLOR_STYLES.get(b.get("color"), (None, None))
            rows.append([make_button(f"🔘 {label}", callback_data="noop", style=preview_style, smallcaps=False)])
            rows.append([
