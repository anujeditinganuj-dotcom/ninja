...
def my_bots_keyboard(bots: list) -> InlineKeyboardMarkup:
    """One bot per row (matches @BotFather's own /mybots layout). This used
    to pack two @username buttons per row — fine for short handles, but
    Telegram sizes every button in a keyboard to the widest one, so two
    longer usernames side by side (e.g. @terabox_downloader_ak_bot) got
    squeezed and visibly cut off. One per row always gets the full row
    width, so the whole handle fits no matter how long it is."""
    rows = [
